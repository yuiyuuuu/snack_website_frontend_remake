import React from "react";
import "./Navbar.css";
import { Link } from "react-router-dom";
// const tempImage =
//   "data:image/png;base64,";

const Navbar = () => {
  return (
    <div className='Navbar'>
      <div className='Navbar--Container' style={{ justifyContent: "center" }}>
        <div
          style={{
            // marginRight: "10px",
            display: "flex",
            flexDirection: "row",
            justifyContent: "center",
            marginLeft: "2%",
            alignItems: "center",
          }}
        >
          <div
            className='Nav--Logo'
            style={{ marginRight: "50px", height: "30px" }}
          >
            <Link to='/' style={{ textDecoration: "none", color: "white" }}>
              {/* <img src={tempImage} alt='logo' width={30} /> */}
            </Link>
          </div>
          <div>
            <Link
              to='/allsnacks'
              style={{ textDecoration: "none", color: "white" }}
            >
              Shop Button
            </Link>
          </div>
          <div style={{ flexGrow: 1 }} />
        </div>

        <div style={{ flexGrow: 1 }} />
        <div style={{ marginLight: "10px" }}>
          BullsEye Store (storenamehere)
        </div>
        <div style={{ flexGrow: 1 }} />
        <div
          style={{
            marginRight: "10px",
            display: "flex",
            flexDirection: "row",
            justifyContent: "center",
            paddingRight: "1%",
            alignItems: "center",
          }}
        >
          <div className='Nav--Profile' style={{}}>
            {/* if user is logged in, or redirect */}
            <Link
              to='/myaccount'
              style={{ textDecoration: "none", color: "white" }}
            >
              Account
            </Link>
          </div>
          <div className='Nav--Cart' style={{ marginRight: "30px" }}>
            Cart
          </div>
          <div className='Nav--Login'>
            <Link
              to='/login'
              style={{ textDecoration: "none", color: "white" }}
            >
              Login/Signup Icon
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navbar;

// /allsnacks - displays all snacks
// /snack/id -displays one snack
// /checkout
